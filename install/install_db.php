...
if (!$input) {
    die(json_encode(['error' => '잘못된 요청 데이터입니다.']));
}

require_once __DIR__ . '/install.function.php';

// 입력값 검증
$dbhost = safe_install_string_check(trim($input['dbhost'] ?? ''), true);
$dbname = safe_install_string_check(trim($input['dbname'] ?? ''), true);
$dbuser = safe_install_string_check(trim($input['dbuser'] ?? ''), true);
$dbpass = safe_install_string_check(trim($input['dbpass'] ?? ''), true);
$bizcode = safe_install_string_check(trim($input['bizcode'] ?? ''), true);
$bizname = safe_install_string_check(trim($input['bizname'] ?? ''), true);

// 필수값 체크
if (!$dbhost || !$dbname || !$dbuser || !$bizcode || !$bizname) {
    die(json_encode(['error' => '필수 입력값이 누락되었습니다.']));
}

// 사업장 코드 유효성 검사
if (!validate_business_code($bizcode)) {
    die(json_encode(['error' => '사업장 코드는 영문자와 숫자로 4자리 이내여야 합니다.']));
}

// 데이터베이스 연결 테스트
$db_check = test_database_connection($dbhost, $dbuser, $dbpass, $dbname);
if (!$db_check['success']) {
    die(json_encode(['error' => $db_check['error']]));
}
...
if (!create_env_file($env_config)) {
    die(json_encode(['error' => '.env 파일 생성에 실패했습니다.']));
}
...
try {
    ...
    // 테이블 생성 SQL 파일 읽기
    $sql_file = file_get_contents(__DIR__ . '/schema.sql');
    if ($sql_file === false) {
    ...
    do {
        if ($mysqli->error) {
            throw new Exception('SQL 실행 오류: ' . $mysqli->error);
        }
    } while ($mysqli->next_result());

    // 기본 관리자 계정 생성 (admin / admin123)
    $admin_password = password_hash('admin123', PASSWORD_DEFAULT);
    $stmt = $mysqli->prepare("INSERT INTO users (username, password, role, business_code, name, created_at) VALUES (?, ?, 'admin', ?, '관리자', NOW())");
    $stmt->bind_param('sss', 'admin', $admin_password, $bizcode);
    $stmt->execute();

    if ($stmt->error) {
        throw new Exception('관리자 계정 생성 실패: ' . $stmt->error);
    }
    ...
    // 실패 시 .env 파일 삭제
    @unlink(__DIR__ . '/../.env');
    die(json_encode(['error' => '설치 중 오류가 발생했습니다: ' . $e->getMessage()]));
}
