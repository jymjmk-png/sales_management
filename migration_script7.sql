...
-- Created: Sun Oct 19 16:19:19 2025
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`우편번호` (
  ...
  INDEX `IX_우편번호` (`우편번호` ASC),
  INDEX `IX_읍면동` (`우편번호` ASC, `읍면동` ASC));
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`자재` (
  ...
  INDEX `IX_자재명` (`자재명` ASC),
  INDEX `IX_바코드` (`자재명` ASC),
  INDEX `IX_규격` (`규격` ASC),
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`자재입출내역` (
  ...
  INDEX `IX_거래일번` (`사업장코드` ASC, `거래일자` ASC, `거래번호` ASC) ,
  INDEX `IX_세금계산서` (`사업장코드` ASC, `작성년도` ASC, `책번호` ASC, `일련번호` ASC),
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`매출세금계산서장부` (
  ...
  INDEX `IX_매출세금계산서번호` (`사업장코드` ASC, `작성년도` ASC, `책번호` ASC, `일련번호` ASC),
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`매입세금계산서장부` (
  ...
  `일련번호` FLOAT(24,0) NOT NULL DEFAULT 0,
  PRIMARY KEY (`사업장코드`, `작성일자`, `작성시간`),
  INDEX `IX_매입세금계산서번호` (`사업장코드` ASC, `작성년도` ASC, `책번호` ASC, `일련번호` ASC),
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`견적` (
  ...
  INDEX `IX_출고일자` (`출고일자` ASC),
  INDEX `IX_출고희망일자` (`출고희망일자` ASC),
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`발주` (
  ...
  INDEX `IX_입고희망일자` (`입고희망일자` ASC),
  INDEX `IX_입고일자` (`입고일자` ASC),
...
CREATE TABLE IF NOT EXISTS `YmhDB`.`세금계산서` (
  ...
  INDEX `IX_작성일자` (`사업장코드` ASC, `작성일자` ASC),
  INDEX `IX_매출처코드` (`사업장코드` ASC, `매출처코드` ASC),
